...
## Playing across different Wi-Fi networks

The server now binds to `0.0.0.0` and uses `process.env.PORT`, so it is ready for internet hosting. Players do **not** need to be on the same Wi-Fi when everyone opens the same public HTTPS game URL.

For local testing, players on another network still cannot reach a computer's `localhost` address directly. Deploy this Node.js project to a public host (such as Render, Railway, Fly.io, or another Node-compatible service), then share that service's HTTPS URL with the host and players. Socket.IO will use the same URL for the multiplayer connection.
