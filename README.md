...
## v1.1 answer-selection updates

- Player-selected answers are highlighted gray until reveal for simultaneous question types.
- After reveal, a player's selected answer turns green when correct and red when incorrect.
- The host screen shows which player selected each answer choice.
- Grab Bag answers are treated as an unordered set: any answer listed in the workbook's `answer` field is correct regardless of position.
- Grab Bag choices can only be claimed once; claimed choices are unavailable to later players.
- Grab Bag selections immediately show green/red on both player and host screens.
- Close Call selections are highlighted gray and become unavailable after being claimed.
