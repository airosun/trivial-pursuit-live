...
function statsPercent(correct, total) {
  return total ? (correct / total) * 100 : 0;
}

function drawStatsBarChart(canvas, playerStats) {
  ...
  const left = 54, right = 20, top = 22, bottom = 72;
  const chartW = width - left - right, chartH = height - top - bottom;
  ctx.strokeStyle = "#d9dce2"; ctx.lineWidth = 1; ctx.fillStyle = "#555b67"; ctx.font = "12px system-ui";
  for (let pct = 0; pct <= 100; pct += 20) {
    const y = top + chartH - (pct / 100) * chartH;
    ctx.beginPath(); ctx.moveTo(left, y); ctx.lineTo(left + chartW, y); ctx.stroke();
    ctx.textAlign = "right"; ctx.fillText(`${pct}%`, left - 8, y + 4);
  }
  const cats = playerStats.categories || [];
  const slot = chartW / Math.max(cats.length, 1);
  const barW = Math.min(72, slot * 0.62);
  cats.forEach((cat, i) => {
    const pct = statsPercent(cat.correct, cat.total);
    const h = chartH * pct / 100;
    const x = left + i * slot + (slot - barW) / 2;
    const y = top + chartH - h;
    ctx.fillStyle = colorValue(cat.color);
    ctx.fillRect(x, y, barW, h);
    ctx.fillStyle = "#17191f"; ctx.font = "800 13px system-ui"; ctx.textAlign = "center";
    ctx.fillText(`${pct.toFixed(1)}%`, x + barW / 2, Math.max(top + 14, y - 7));
    ctx.font = "800 12px system-ui";
    const words = String(cat.name || cat.color).split(/\s+/);
    words.forEach((word, wi) => ctx.fillText(word, x + barW / 2, top + chartH + 22 + wi * 14));
  });
  ctx.save(); ctx.translate(13, top + chartH / 2); ctx.rotate(-Math.PI / 2); ctx.textAlign = "center"; ctx.font = "800 12px system-ui"; ctx.fillStyle = "#555b67"; ctx.fillText("Percentage Correct", 0, 0); ctx.restore();
}

function drawStatsPieChart(canvas, playerStats) {
  const dpr = window.devicePixelRatio || 1;
  const rect = canvas.getBoundingClientRect();
  const width = Math.max(520, Math.floor(rect.width || 760));
  const height = 360;
  ...
  const cats = playerStats.categories || [];
  const total = cats.reduce((sum, c) => sum + (c.total || 0), 0);
  const cx = Math.min(width * 0.35, 230), cy = 170, radius = 125;
  if (!total) { ctx.fillStyle = "#666c78"; ctx.font = "700 16px system-ui"; ctx.textAlign = "center"; ctx.fillText("No questions answered", cx, cy); return; }
  let start = -Math.PI / 2;
  cats.forEach(cat => {
    const value = cat.total || 0; if (!value) return;
    const end = start + Math.PI * 2 * value / total;
    ctx.beginPath(); ctx.moveTo(cx, cy); ctx.arc(cx, cy, radius, start, end); ctx.closePath();
    ctx.fillStyle = colorValue(cat.color); ctx.fill();
    ctx.strokeStyle = "#fff"; ctx.lineWidth = 2; ctx.stroke();
    start = end;
  });
  const lx = Math.min(width * 0.58, 430);
  cats.forEach((cat, i) => {
    const y = 48 + i * 42;
    ctx.fillStyle = colorValue(cat.color); ctx.fillRect(lx, y - 13, 18, 18);
    ctx.fillStyle = "#17191f"; ctx.font = "800 13px system-ui"; ctx.textAlign = "left";
    ctx.fillText(`${cat.name} — ${cat.total || 0}`, lx + 27, y + 1);
  });
}
...
function renderStatsModal() {
  ...
  if (!stats.length) { content.innerHTML = '<div class="stats-empty">No player statistics are available.</div>'; return; }
  content.innerHTML = stats.map((player, index) => {
    const mode = statsChartModes[player.playerId] || "bar";
    return `<section class="stats-player"><div class="stats-player-title"><h3>${esc(player.playerName)}</h3><span>${player.categories.reduce((a,c)=>a+c.total,0)} questions answered</span></div><canvas class="stats-canvas" data-stats-player="${esc(player.playerId)}"></canvas><button class="stats-toggle" data-stats-toggle="${esc(player.playerId)}">Toggle</button></section>`;
  }).join("");
  stats.forEach(player => {
    const canvas = content.querySelector(`[data-stats-player="${CSS.escape(player.playerId)}"]`);
    if ((statsChartModes[player.playerId] || "bar") === "pie") drawStatsPieChart(canvas, player); else drawStatsBarChart(canvas, player);
  });
  content.querySelectorAll("[data-stats-toggle]").forEach(btn => btn.onclick = () => {
    const id = btn.dataset.statsToggle; statsChartModes[id] = (statsChartModes[id] || "bar") === "bar" ? "pie" : "bar"; renderStatsModal();
  });
}

function openStats() {
  if (!state || state.status !== "finished" || !state.playerStats) return;
  $("statsModal").hidden = false; renderStatsModal();
}
...
$("showStats").onclick = openStats;
$("closeStats").onclick = () => { $("statsModal").hidden = true; };
$("statsModal").querySelector(".stats-modal-backdrop").onclick = () => { $("statsModal").hidden = true; };
...
function snapshotVisualState(s) {
  ...
  return {
    ...
    wedgeAwards: (s.wedgeAwards || []).map(a => ({ id:a.id, playerId:a.playerId, playerName:a.playerName, color:a.color })),
...
function updateVisualTransitions(prev, next) {
  ...
  // Wedge awards are explicit one-time events. Process them before any
  // transition early-returns so a round/category change cannot suppress them.
  const oldAwardIds = new Set((prev.wedgeAwards || []).map(a => a.id));
  const playerOrder = new Map((next.scores || []).map((p, i) => [p.id, i]));
  const newAwards = (next.wedgeAwards || [])
    .filter(a => !oldAwardIds.has(a.id))
    .sort((a, b) => (playerOrder.get(a.playerId) ?? Number.MAX_SAFE_INTEGER) - (playerOrder.get(b.playerId) ?? Number.MAX_SAFE_INTEGER));
  if (role === 'host' && newAwards.length) {
    queueWedgeAwards(newAwards);
  } else {
    newAwards.forEach(a => highlightPlayer(a.playerId));
  }
  ...
  if (next.status === 'finished' && prev.status !== 'finished') {
    ...
    // If wedges were awarded in the same update that produced the winner,
    // wait until every wedge pop-up has completely finished before showing
    // the winner celebration. This guarantees: wedge 1 -> wedge 2 -> winner.
    if (role === 'host' && (newAwards.length || wedgeAwardPlaying || wedgeAwardQueue.length)) {
      queueAfterWedgeAwards(finishCelebration);
    } else {
      finishCelebration();
    }
    return;
...
const wedgeAwardQueue = [];
let wedgeAwardPlaying = false;
const postWedgeCallbacks = [];

function queueAfterWedgeAwards(callback) {
  if (typeof callback !== 'function') return;
  postWedgeCallbacks.push(callback);
  playNextWedgeAward();
}

function finishWedgeQueueIfIdle() {
  if (wedgeAwardPlaying || wedgeAwardQueue.length) return;
  while (postWedgeCallbacks.length) {
    const callback = postWedgeCallbacks.shift();
    try { callback(); } catch {}
  }
}

function queueWedgeAwards(awards) {
  awards.forEach(award => {
    wedgeAwardQueue.push(award);
    highlightPlayer(award.playerId);
  });
  playNextWedgeAward();
}

function playNextWedgeAward() {
  if (wedgeAwardPlaying || !wedgeAwardQueue.length) {
    finishWedgeQueueIfIdle();
    return;
  }
  wedgeAwardPlaying = true;
  const award = wedgeAwardQueue.shift();
  showWedgeAward(award.playerName, award.color, () => {
    wedgeAwardPlaying = false;
    if (wedgeAwardQueue.length) playNextWedgeAward();
    else finishWedgeQueueIfIdle();
  });
}

function showWedgeAward(name, wedge, onDone) {
...
function renderHost() {
  ...
  $("hostCode").classList.toggle("room-code-hidden", roomCodeHidden);
  $("toggleRoomCode").textContent = roomCodeHidden ? "Show Room Code" : "Hide Room Code";
  $("hostControls").hidden = false;
  ...
  $("showStats").hidden = state.status !== "finished";
  ...
  if (!(state.round === "Close Call" && state.revealed && Array.isArray(state.answer))) {
    if (state.revealed) {
      $("hostReveal").innerHTML = formatAnswer(state);
    } else {
      $("hostReveal").textContent = state.notice || (state.turnPlayerName && ["Grab Bag","Close Call"].includes(state.round) ? `Waiting for ${state.turnPlayerName}.` : "Answers are being collected.");
    }
  }
...
function renderPlayer() {
  ...
  if (state.revealed) setStatus("playerStatus", formatPlayerAnswer(state), false);
...
function formatAnswer(s) {
  ...
  if (Array.isArray(s.answer)) {
    if (s.round === "Grab Bag") return `<strong>Correct Options</strong><div class="correct-options"><strong>${s.answer.map(esc).join(", ")}</strong></div>`;
    return `Correct order: ${s.answer.map(esc).join(" → ")}`;
  }
  return `Correct answer: ${esc(s.answer)}`;
}

function formatPlayerAnswer(s) {
  if (!s.answer) return "";
  if (Array.isArray(s.answer)) {
    if (s.round === "Grab Bag") return `Correct Options: ${s.answer.join(", ")}`;
    return `Correct order: ${s.answer.join(" → ")}`;
  }
