...
function renderPlayer() {
  ...
  $("joinPanel").hidden = isPlaying;
  const isSwitch = state.round === "Switchagories";
  // Keep the player game area visible throughout the live game, including
  // round-intro/category-preview/question-preview stages where there is no
  // question object yet. Hiding it here made the player side appear blank
  // as soon as the game started.
  if (!state.question && !isSwitch && state.status !== "finished" && state.status !== "playing") { $("playerGame").hidden = true; return; }
  ...
  if (state.revealed) {
    $("playerStatus").innerHTML = formatAnswer(state);
