...
function renderHost() {
  ...
  const optionStateClass = value => {
    ...
    if (!picked.length) return "";
    if (state.round === "Close Call") return state.revealed ? "close-call-picked" : "selected-gray";
  ...
  if (state.round === "Close Call" && state.revealed && Array.isArray(state.answer)) {
    const orderText = state.answer.map((a, i) => `${i + 1}. ${esc(a)}`).join("  →  ");
...
function renderPlayer() {
  ...
  const closeCallReveal = state.round === "Close Call" && state.revealed && Array.isArray(state.answer)
    ? `<div class="close-call-reveal"><strong>CORRECT ORDER</strong><div>${state.answer.map((a, i) => `${i + 1}. ${esc(a)}`).join("  →  ")}</div></div>`
    : "";
  ...
  const playerOptionClass = value => {
    ...
    if (mine) {
      if (state.round === "Grab Bag") return mine.correct ? "selected-correct" : "selected-wrong";
  ...
  $("playerAnswers").innerHTML = state.question.options.map((x,i) => {
    ...
    const claimedBySelf = Object.prototype.hasOwnProperty.call(claimedBy, x) && claimedBy[x] === myPlayerId;
