...
function renderHost() {
  ...
  const optionStateClass = value => {
    ...
    if (state.round === "Grab Bag") return picked[0].correct ? "selected-correct" : "selected-wrong";
    if (state.round === "Close Call") return "selected-gray";
    if (!state.revealed) return "selected-gray";
    return picked.some(a => a.correct) ? "selected-correct" : "selected-wrong";
  };
  const optionPlayers = value => selectionFor(value).map(a => `<span class="answer-picker">${esc(a.playerName)}</span>`).join("");
  $("hostAnswers").innerHTML = state.question.options.map((x,i) => {
    const picked = selectionFor(x);
    return `<div class="answer static-answer ${optionStateClass(x)}">${i+1}. ${esc(x)}${picked.length ? `<div class="answer-pickers">${optionPlayers(x)}</div>` : ""}</div>`;
  }).join("");
...
function renderPlayer() {
  ...
  const mySelection = (state.answerSelections || []).find(a => a.playerId === myPlayerId)?.value || null;
  const claimedBy = state.claimedAnswers || {};
  const disabled = !myTurn || state.revealed || finalEliminated;
  const playerOptionClass = value => {
    const selected = mySelection === value;
    const claimed = Object.prototype.hasOwnProperty.call(claimedBy, value);
    if (selected) {
      if (state.round === "Grab Bag") {
        const mine = (state.answerSelections || []).find(a => a.playerId === myPlayerId && a.value === value);
        return mine?.correct ? "selected-correct" : "selected-wrong";
      }
      if (state.round === "Close Call" || !state.revealed) return "selected-gray";
      const mine = (state.answerSelections || []).find(a => a.playerId === myPlayerId && a.value === value);
      return mine?.correct ? "selected-correct" : "selected-wrong";
    }
    if ((state.round === "Grab Bag" || state.round === "Close Call") && claimed) return "claimed-gray";
    return "";
  };
  $("playerAnswers").className = "answers " + (state.round === "Grab Bag" ? "grab-grid" : "");
  $("playerAnswers").innerHTML = state.question.options.map((x,i) => {
    const claimedByOther = Object.prototype.hasOwnProperty.call(claimedBy, x) && claimedBy[x] !== myPlayerId;
    const optionDisabled = disabled || claimedByOther;
