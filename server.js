...
function recordWedgeAward(game, playerId, color) {
  game.wedgeAwardSeq = (game.wedgeAwardSeq || 0) + 1;
  game.wedgeAwards = game.wedgeAwards || [];
  const p = game.players.get(playerId);
  game.wedgeAwards.push({ id: game.wedgeAwardSeq, playerId, playerName: p?.name || "Player", color });
  // Keep only the recent events; clients only need the latest award(s).
  if (game.wedgeAwards.length > 12) game.wedgeAwards.splice(0, game.wedgeAwards.length - 12);
}
...
function checkScoreWedges(game, playerId, category) {
  ...
  while (p.scoreWedgesAwarded < WEDGE_THRESHOLDS.length && p.score >= WEDGE_THRESHOLDS[p.scoreWedgesAwarded]) {
    ...
    recordWedgeAward(game, playerId, color);
  }
}
...
function parseWorkbook(buffer) {
  ...
  const questions = rows.map((row, index) => {
    ...
    const optionCount = type === "multi-choice" ? 4 :
      type === "close-call" ? 5 :
      type === "grab-bag" ? 16 :
      type === "final-round" ? 2 : 0;
    const options = [];
    for (let i = 1; i <= optionCount; i++) {
      const value = clean(row[`op${i}`]);
      if (value) options.push(value);
    }
    // Close Call answer values live in the five cells immediately after the
    // answer choices: op1/op6, op2/op7, op3/op8, op4/op9, op5/op10.
    const closeCallValues = [];
    if (type === "close-call") {
      for (let i = 6; i <= 10; i++) closeCallValues.push(clean(row[`op${i}`]));
    }
    return {
      row: index + 2,
      id: clean(row.id),
...
function publicGame(game, viewerId = null) {
  return {
    ...
    answerValues: game.revealed && game.currentPublicQuestion?.type === "close-call" ? (game.currentCloseCallValues || []) : null,
    ...
    stageStartedAt: game.stageStartedAt || null,
    stageEndsAt: game.stageEndsAt || null,
    scores: [...game.players.values()].map(p => ({ id:p.id, name:p.name, score:p.score, wedges:p.wedges || [], wedgesList:p.wedges || [] })),
    wedgeAwards: game.wedgeAwards || [],
    playerStats: game.status === "finished" && viewerId === null ? publicPlayerStats(game) : null,
    ...
    answerSelections: (() => {
      ...
      if (round === "Grab Bag") {
        return (game.selectionHistory || []).map(s => ({
          playerId: s.playerId,
      ...
      const selections = [...game.answers.entries()].filter(([, value]) => value).map(([playerId, value]) => ({
        playerId,
        playerName: game.players.get(playerId)?.name || "Player",
        value,
        correct: game.revealed ? (round === "Close Call"
          ? game.currentAnswer.includes(value)
          : value === game.currentAnswer) : null
      }));
      // During simultaneous rounds, hide everyone else's selections from the
      // host and from other players until the answer is revealed. Each player
      // still receives their own selection so their button can turn gray.
      if (!game.revealed && ["Quickstarter", "Switchagories", "Final Round"].includes(round)) {
        return viewerId ? selections.filter(s => s.playerId === viewerId) : [];
      }
      return selections;
    })(),
...
function makePublicQuestion(q, round, finalOrder) {
  ...
  const optionValues = q.type === "close-call"
    ? Object.fromEntries(options.map(option => {
        const index = q.options.indexOf(option);
        return [option, q.closeCallValues?.[index] ?? ""];
      }))
    : null;
...
function beginQuestionPresentation(game) {
  stopTimer(game);
  const round = game.rounds[game.roundIndex];
  // Final Round shows its category once at the start of the five-question set.
  // Questions 2-5 go straight to the question preview.
  if (round === "Switchagories" || round === "Final Round") {
    setStage(game, "question-preview");
  } else {
    setStage(game, "category-preview", 2000);
  }
}

function activateQuestion(game) {
  if (!game.currentPublicQuestion || game.revealed) return;
  setStage(game, "answering");
  startTimer(game);
}
...
function recordPlayerStat(game, playerId, category, correct) {
  const player = game.players.get(playerId);
  if (!player || !category) return;
  game.playerStats ||= new Map();
  if (!game.playerStats.has(playerId)) game.playerStats.set(playerId, new Map());
  const byCategory = game.playerStats.get(playerId);
  const key = normalizeCategory(category);
  const existing = byCategory.get(key) || { total: 0, correct: 0 };
  existing.total += 1;
  if (correct) existing.correct += 1;
  byCategory.set(key, existing);
}

function publicPlayerStats(game) {
  return [...game.players.values()].map(player => {
    const byCategory = game.playerStats?.get(player.id) || new Map();
    return {
      playerId: player.id,
      playerName: player.name,
      categories: game.categories.map(category => {
        const stat = byCategory.get(normalizeCategory(category.name)) || { total: 0, correct: 0 };
        return { name: category.name, color: category.color, total: stat.total, correct: stat.correct };
      })
    };
  });
}
...
function revealQuestion(game) {
  ...
  if (round === "Quickstarter") {
    ...
    for (const [playerId, answer] of game.answers.entries()) {
      const correct = answer === game.currentAnswer;
      recordPlayerStat(game, playerId, game.currentPublicQuestion.category, correct);
      if (correct) addScore(game, playerId, points, game.currentPublicQuestion.category);
    }
  } else if (round === "Switchagories") {
    // The player who selected the category gets 1800 for a correct answer.
    // Every other player who answers correctly gets 900.
    const categoryPickerId = game.switchagoriesTurn?.playerId;
    for (const [playerId, answer] of game.answers.entries()) {
      const correct = answer === game.currentAnswer;
      recordPlayerStat(game, playerId, game.currentPublicQuestion.category, correct);
      if (correct) {
        const points = playerId === categoryPickerId ? 1800 : 900;
        addScore(game, playerId, points, game.currentPublicQuestion.category);
      }
    }
    ...
    // The fifth question ends the category. Any players still standing at that
    // point are winners of the category and receive its wedge.
    if (game.questionIndex === game.currentQuestions.length - 1 && !game.finalCategoryResolved) {
      const standing = [...game.players.keys()].filter(id => !game.finalEliminated.has(id));
      for (const playerId of standing) awardFinalWedge(game, playerId, category);
      game.finalCategoryResolved = true;
      game.finalCategoryWinner = standing.length === 1 ? standing[0] : null;
    }

    // If one player remains, award the category wedge and resolve it.
    ...
    else if (game.finalCategoryResolved) { game.notice = "Category complete — press Next for the next category."; }
  } else if (round === "Close Call") {
    const points = game.roundIndex === 1
      ? [1200, 1000, 800, 600, 0]
      : [2400, 2000, 1600, 1200, 0];
    for (const [playerId, answer] of game.answers.entries()) {
      const correct = answer === game.currentAnswer?.[0];
      recordPlayerStat(game, playerId, game.currentPublicQuestion.category, correct);
      if (!answer) continue;
...
function startQuestion(game) {
  ...
  game.currentAnswer = q.type === "close-call"
    ...
    : q.type === "grab-bag"
      ? parsePipe(q.answer)
      : q.answer;
  game.currentCloseCallValues = q.type === "close-call"
    ? (game.currentPublicQuestion.options || []).map(option => {
        const index = q.options.indexOf(option);
        return q.closeCallValues?.[index] ?? "";
      })
    : null;
  game.revealed = false;
  game.selectionHistory = [];
  ...
  if (game.rounds[game.roundIndex] === "Switchagories") {
    ...
    game.currentAnswer = null;
    stopTimer(game);
    setStage(game, "switch-category");
  } else {
    beginQuestionPresentation(game);
  }
}

function uniqueCategoryQuestions(game, type, count) {
  const byCategory = new Map();
  const used = game.usedQuestionIds || new Set();
  for (const q of game.questionBank.filter(q => q.type === type && !used.has(q.id))) {
  ...
  if (categories.length < count) {
    throw new Error(`${type} needs at least ${count} unused different categories for this game.`);
  }
  const selected = categories.slice(0, count).map(category => shuffle(byCategory.get(category))[0]);
  for (const q of selected) used.add(q.id);
...
function questionsForRound(game, round) {
  ...
  if (round === "Final Round") {
    ...
    const used = game.usedQuestionIds || new Set();
    const categories = shuffle([...byCategory.keys()]);
    const availableGroups = categories.flatMap(category =>
      shuffle(byCategory.get(category)).filter(group => !used.has(group.id)).slice(0, 1)
    );
    if (availableGroups.length < 6) throw new Error("Final Round needs 6 unused category sets for this game.");
    game.finalGroups = availableGroups.slice(0, 6);
    for (const group of game.finalGroups) used.add(group.id);
    game.usedQuestionIds = used;
...
function setSwitchagoriesCategoryChoices(game) {
  const available = game.categories.filter(c =>
    game.questionBank.some(q => q.type === "multi-choice" && q.category === normalizeCategory(c.name))
  );
  game.switchagoriesCategoryChoices = shuffle(available).slice(0, 2).map(c => ({
    color: c.color,
    name: c.name
  }));
}
...
function startSwitchagoriesRound(game) {
  ...
  initializeTurnState(game);
  game.switchagoriesTurn = { playerId: game.turnOrder[0] || null, category: null };
  setSwitchagoriesCategoryChoices(game);
  stopTimer(game);
  game.timerSeconds = 0;
  game.timerEndsAt = null;
  setStage(game, "switch-category");
...
function queueFinalCategoryAdvance(game) {
  // Final Round category changes are deliberately manual: the host presses Next
  // after the wedge/result fanfare has been shown.
  return;
}
...
function finishRound(game) {
  ...
  if (game.rounds[game.roundIndex] === "Switchagories") {
    ...
    // Prepare the first question but do not reveal its choices or start its timer
    // until the host advances through the round/question presentation.
    startQuestion(game);
    setStage(game, "round-intro");
    stopTimer(game);
  }
...
function emitState(game) {
  for (const socket of io.sockets.sockets.values()) {
    if (!socket.rooms.has(game.code)) continue;
    socket.emit("state", publicGame(game, socket.data.playerId || null));
  }
}
...
io.on("connection", socket => {
  socket.on("host:create", ({ code, hostToken } = {}, ack = () => {}) => {
    ...
    ack({ ok: true, state: publicGame(game, socket.data.playerId || null) });
    emitState(game);
  });
  ...
  socket.on("player:join", ({ code, name, reconnectToken } = {}, ack = () => {}) => {
    ...
    if (reconnectToken) {
      ...
      if (existing) {
        ...
        ack({ ok: true, playerId: existing.id, reconnectToken: existing.reconnectToken, state: publicGame(game, socket.data.playerId || null), reconnected: true });
        emitState(game);
        return;
      }
    }
    ...
    const existingByName = [...game.players.values()].find(p => p.name.toLowerCase() === playerName.toLowerCase());
    if (existingByName) {
      if (existingByName.connected) return ack({ ok: false, error: "That player name is already taken." });
      existingByName.connected = true;
      existingByName.socketId = socket.id;
      const pending = game.reconnectTimers.get(existingByName.id);
      if (pending) clearTimeout(pending);
      game.reconnectTimers.delete(existingByName.id);
      socket.join(game.code);
      socket.data.gameCode = game.code;
      socket.data.role = "player";
      socket.data.playerId = existingByName.id;
      return ack({ ok: true, playerId: existingByName.id, reconnectToken: existingByName.reconnectToken, state: publicGame(game, existingByName.id), reconnected: true });
    }
    ...
    ack({ ok: true, playerId, reconnectToken: reconnectTokenForPlayer, state: publicGame(game, playerId) });
    emitState(game);
  });

  socket.on("host:remove-player", ({ playerId } = {}, ack = () => {}) => {
    const game = games.get(socket.data.gameCode);
    if (!game || game.hostId !== socket.id) return ack({ ok: false, error: "Not authorized." });
    if (game.status !== "lobby") return ack({ ok: false, error: "Players can only be removed from the lobby." });
    const player = game.players.get(playerId);
    if (!player) return ack({ ok: false, error: "Player not found." });
    if (player.socketId) {
      const playerSocket = io.sockets.sockets.get(player.socketId);
      if (playerSocket) {
        playerSocket.emit("player:removed", { message: "The host removed you from the lobby." });
        playerSocket.leave(game.code);
        playerSocket.data.gameCode = null;
        playerSocket.data.playerId = null;
      }
    }
    game.players.delete(playerId);
    const pending = game.reconnectTimers.get(playerId);
    if (pending) clearTimeout(pending);
  ...
  socket.on("player:switch-category", ({ category } = {}, ack = () => {}) => {
    ...
    const categoryKey = normalizeCategory(category);
    if (!game.switchagoriesCategoryChoices.some(c => normalizeCategory(c.name) === categoryKey)) {
      return ack({ ok: false, error: "That category was not one of your two choices." });
    }
  ...
  socket.on("player:answer", ({ value } = {}, ack = () => {}) => {
    ...
    if ((round === "Grab Bag" || round === "Close Call") && game.turnOrder[game.turnIndex] !== socket.data.playerId) {
      return ack({ ok: false, error: "It is not your turn." });
    }
    ...
    if (round === "Grab Bag") {
      ...
      recordPlayerStat(game, socket.data.playerId, game.currentPublicQuestion.category, correct);
      ...
      game.selectionNotice = `${player.name} selected ${answer}${correct ? " — CORRECT" : " — INCORRECT"}`;
      ...
      // Once every correct option has been claimed, reveal the whole board automatically.
      const allCorrectClaimed = game.currentAnswer.every(correctValue => Object.prototype.hasOwnProperty.call(game.claimedAnswers, correctValue));
      if (allCorrectClaimed) {
        revealQuestion(game);
        return;
      }
    ...
    if (round === "Switchagories") {
      if (game.answers.has(socket.data.playerId)) {
        return ack({ ok: false, error: "You have already answered this question." });
      }
      game.answers.set(socket.data.playerId, answer);
      ack({ ok: true, result: "selected" });
      if (game.answers.size >= game.players.size) revealQuestion(game);
      else emitState(game);
      return;
    }
  ...
  socket.on("host:next", (_, ack = () => {}) => {
    ...
    if (game.status === "playing" && game.stage === "round-intro") {
      if (round === "Final Round") {
        game.notice = `Final Round: ${game.finalCategoryName || game.finalGroups[game.finalCategoryIndex]?.category || "Category"}`;
      }
      beginQuestionPresentation(game);
      ack({ ok: true });
      emitState(game);
    ...
    if (game.status === "playing" && game.stage === "question-preview") {
      activateQuestion(game);
      ack({ ok: true });
      emitState(game);
      return;
    }
    if (game.status === "playing" && game.stage === "category-preview") {
      setStage(game, "question-preview");
      ack({ ok: true });
      emitState(game);
      return;
    }
    if (game.status === "playing" && game.stage === "switch-category" && round === "Switchagories") {
      ack({ ok: false, error: "Have the player choose a category first." });
      return;
    }
...
server.listen(PORT, "0.0.0.0", () => {
  console.log(`Trivial Pursuit Live server running on port ${PORT}`);
});