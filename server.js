...
function publicGame(game) {
  return {
    ...
    finalCategoryName: game.finalGroups?.[game.finalCategoryIndex]?.category || null,
    ...
    answerSelections: [...game.answers.entries()].map(([playerId, value]) => ({
      playerId,
      playerName: game.players.get(playerId)?.name || "Player",
      value,
      correct: game.rounds[game.roundIndex] === "Grab Bag"
        ? game.currentAnswer.includes(value)
        : (game.revealed ? (game.rounds[game.roundIndex] === "Close Call"
          ? game.currentAnswer.includes(value)
          : value === game.currentAnswer) : null)
    })),
    claimedAnswers: [...game.answers.entries()].reduce((out, [playerId, value]) => {
      out[value] = playerId;
      return out;
    }, {})
  };
...
function startTimer(game) {
  ...
  game.timerHandle = setTimeout(() => {
    ...
    if (round === "Grab Bag") {
      ...
      // Close Call timeouts simply forfeit the player's selection; their turn
      // advances without claiming an answer choice.
      advanceTurn(game);
    }
...
function revealQuestion(game) {
  ...
  if (round === "Quickstarter") {
    ...
    const correct = new Set(game.currentAnswer);
    for (const [playerId, answer] of game.answers.entries()) {
      if (correct.has(answer)) addScore(game, playerId, points, game.currentPublicQuestion.category);
    }
  }
...
app.post("/api/games/:code/questions", upload.single("workbook"), (req, res) => {
  ...
  try {
    ...
    game.questionBank = parsed.questions;
...
io.on("connection", socket => {
  ...
  socket.on("player:answer", ({ value } = {}, ack = () => {}) => {
    ...
    if (round === "Grab Bag") {
      // A Grab Bag choice can only be claimed once. The answer list itself is
      // intentionally unordered: any option listed in the workbook's answer
      // field is correct, regardless of its position.
      ...
      if (!game.currentAnswer.includes(answer)) {
    ...
    if (round === "Close Call") {
      ...
      game.answers.set(socket.data.playerId, answer);
      game.currentPublicQuestion.options = game.currentPublicQuestion.options.filter(option => option !== answer);
  ...
  socket.on("host:categories", ({ categories } = {}, ack = () => {}) => {
    ...
    game.categories = categories.slice(0, 6).map((c, i) => ({
